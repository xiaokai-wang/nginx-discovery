...
#define NGX_FAKE_PORT  11111
#define NGX_FAKE_IP    "127.0.0.1"

#define NGX_CONFIG_NUM 8


typedef struct {
    ngx_uint_t           add_server;
} ngx_http_upstream_server_loc_conf_t;
...
typedef struct {
    ngx_array_t          upstream_server;
} ngx_http_upstream_server_ctx_t;
...
static char *ngx_http_upstream_set_add_server(ngx_conf_t *cf, ngx_command_t *cmd, 
             void *conf);

static void *ngx_http_upstream_server_create_loc_conf(ngx_conf_t *cf);
static char *ngx_http_upstream_server_merge_loc_conf(ngx_conf_t *cf, 
             void *parent, void *child);

static ngx_int_t ngx_http_upstream_server_init_process(ngx_cycle_t *cycle);
static ngx_int_t ngx_http_upstream_server_handler(ngx_http_request_t *r);

static void ngx_http_upstream_server_init(ngx_http_request_t *r);
static void ngx_http_upstream_pass_server_info(ngx_cycle_t *cycle, ngx_channel_t *ch);

ngx_int_t ngx_http_upstream_add_server(ngx_cycle_t *cycle, ngx_channel_t *ch);
ngx_int_t ngx_http_upstream_copy_peer(ngx_cycle_t *cycle,
        ngx_http_upstream_srv_conf_t *uscf, ngx_uint_t num);
ngx_int_t ngx_http_upstream_add_peer(ngx_cycle_t *cycle, 
        ngx_http_upstream_srv_conf_t *uscf, ngx_http_upstream_server_t *us);

static void ngx_http_upstream_add_check(ngx_cycle_t *cycle, ngx_channel_t *ch);
static ngx_int_t ngx_http_upstream_server_parse_json(u_char *buf, ngx_channel_t *ch);

static void ngx_http_upstream_del_fake_peer(ngx_cycle_t * cycle, 
        ngx_http_upstream_srv_conf_t *uscf);
static ngx_int_t ngx_http_upstream_del_fake_peer_check(ngx_cycle_t *cycle,
        ngx_http_upstream_srv_conf_t *uscf);

ngx_int_t ngx_http_upstream_del_server(ngx_cycle_t *cycle, ngx_channel_t *ch);
static ngx_int_t ngx_http_upstream_del_peer(ngx_cycle_t *cycle,
        ngx_http_upstream_srv_conf_t *uscf, ngx_http_upstream_server_t *us);
static void ngx_http_upstream_del_check(ngx_cycle_t *cycle, ngx_channel_t *ch);

static void ngx_http_upstream_add_server_event_init(ngx_http_upstream_rr_peers_t *tmp_peers, 
        ngx_http_upstream_host_t *upstream_host, ngx_flag_t flag);
static void ngx_http_upstream_add_server_add_delay_delete(ngx_event_t *event);
static void ngx_http_upstream_del_server_del_delay_delete(ngx_event_t *event);

static void ngx_http_upstream_dynamic_free(ngx_http_upstream_rr_peers_t *peers);

void ngx_http_finalize_request_success(ngx_http_request_t *r);
void ngx_http_finalize_request_failed(ngx_http_request_t *r);


static char *upstream_config[NGX_CONFIG_NUM] = {"upstream","server","weight",
                                             "max_fails","fail_timeout","down","backup", "method"};
static char *add_failed = "{\"upstream server operation failed\"}";
static char *add_succeed = "{\"upstream server operation succeed\"}";

static ngx_http_upstream_server_ctx_t *upstream_server = NULL;

static char *fake_peer = "127.0.0.1:11111";

static ngx_command_t  ngx_http_upstream_server_commands[] = {

    { ngx_string("upstream_add_server"),
      NGX_HTTP_LOC_CONF|NGX_CONF_FLAG,
      ngx_http_upstream_set_add_server,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_upstream_server_loc_conf_t, add_server),
      NULL },
...
static ngx_http_module_t  ngx_http_upstream_server_module_ctx = {
    ...
    ngx_http_upstream_server_create_loc_conf,     /* create location configuration */
    ngx_http_upstream_server_merge_loc_conf       /* merge location configuration */
};


ngx_module_t  ngx_http_upstream_server_module = {
    NGX_MODULE_V1,
    &ngx_http_upstream_server_module_ctx,   /* module context */
    ngx_http_upstream_server_commands,      /* module directives */
    NGX_HTTP_MODULE,                            /* module type */
    NULL,                                       /* init master */
    NULL,                                       /* init module */
    ngx_http_upstream_server_init_process,      /* init process */
    NULL,                                       /* init thread */
    NULL,                                       /* exit thread */
    NULL,                                       /* exit process */
    NULL,                                       /* exit master */
...
static char *
ngx_http_upstream_set_add_server(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    char *ret;

    ngx_http_core_loc_conf_t                 *clcf;
    ngx_http_upstream_server_loc_conf_t      *uslcf = conf;
    ...
    if(uslcf->add_server){
        ...
        clcf->handler = ngx_http_upstream_server_handler;
    }
...
static ngx_int_t
ngx_http_upstream_server_handler(ngx_http_request_t *r)
{
    ngx_int_t                              rc;
    ...
    rc = ngx_http_read_client_request_body(r, ngx_http_upstream_server_init);
...
static void
ngx_http_upstream_server_init(ngx_http_request_t *r)
{
    ngx_buf_t                             *buf;
    ngx_channel_t                         *ch;

    ch = ngx_pcalloc(r->pool, sizeof(ngx_channel_t));

    buf = r->request_body->bufs->buf;

    *(buf->last) = '\0';

    if (ngx_http_upstream_server_parse_json(buf->pos, 
                                                ch) == NGX_ERROR) {
    ...
    if (ch->command == NGX_CMD_ADD_SERVER) {
        ...
        ngx_http_upstream_add_check((ngx_cycle_t *)ngx_cycle, ch);

        if (ngx_http_upstream_add_server((ngx_cycle_t *)ngx_cycle, 
                                          ch) != NGX_OK) {
        ...
        ngx_http_upstream_del_check((ngx_cycle_t *)ngx_cycle, ch);

        if (ngx_http_upstream_del_server((ngx_cycle_t *)ngx_cycle, 
                                          ch) != NGX_OK) {
    ...
    ngx_http_upstream_pass_server_info((ngx_cycle_t *)ngx_cycle, ch);
...
static void
ngx_http_upstream_pass_server_info(ngx_cycle_t *cycle, ngx_channel_t *ch)
{
    ngx_int_t  i;
    ngx_pid_t  pid;
    ngx_core_conf_t   *ccf;
    ...
    for (i = 0; i < ccf->worker_processes; i++) {
        ...
        ngx_log_debug6(NGX_LOG_DEBUG, cycle->log, 0,
                      "pass server info s:%d pid:%P fd:%d to s:%i pid:%P fd:%d",
...
ngx_int_t
ngx_http_upstream_add_server(ngx_cycle_t *cycle, ngx_channel_t *ch)
{
    time_t                           fail_timeout;
    u_char                          *port, *p, *last;
    ...
    if (ngx_http_upstream_copy_peer(cycle, uscf, j) != NGX_OK) {
        goto invalid;
    }

    if (ngx_http_upstream_add_peer(cycle, uscf, &us) != NGX_OK) {
        goto invalid;
    }
...
ngx_int_t
ngx_http_upstream_copy_peer(ngx_cycle_t *cycle,
    ngx_http_upstream_srv_conf_t *uscf, ngx_uint_t num)
{
    ngx_uint_t                     i, n, m, len;
    ngx_http_upstream_host_t      *upstream_host = NULL;
    ...
    for (i = 0; i < upstream_server->upstream_server.nelts; i++) {
        upstream_host = (ngx_http_upstream_host_t *)upstream_server->upstream_server.elts + i;
    ...
    if (tmp_peers) {
        ...
        if (n == 0) {
            if (upstream_host->update_label) {
                ngx_http_upstream_add_server_event_init(tmp_peers, upstream_host, NGX_ADD);
            }
...
invalid:
    ...
    if (peers != NULL) {
        ngx_http_upstream_dynamic_free(peers);
    }

    if (backup != NULL) {
        ngx_http_upstream_dynamic_free(backup);
    }
...
ngx_int_t
ngx_http_upstream_add_peer(ngx_cycle_t *cycle,
    ngx_http_upstream_srv_conf_t *uscf, ngx_http_upstream_server_t *us)
{
    ngx_uint_t                     i, n, w, m;
    ngx_http_upstream_host_t      *upstream_host = NULL;
    ...
    if (!upstream_host->update_label) {
        ngx_http_upstream_del_fake_peer(cycle, uscf);
...
static void
ngx_http_upstream_add_check(ngx_cycle_t *cycle, ngx_channel_t *ch)
{
    ngx_uint_t                       i, j;
    ngx_http_upstream_rr_peers_t    *peers=NULL;
    ngx_http_upstream_srv_conf_t    *uscf, **uscfp;
...
static void
ngx_http_upstream_del_fake_peer(ngx_cycle_t *cycle, 
        ngx_http_upstream_srv_conf_t *uscf)
{
    ngx_int_t                                port;
    ngx_pool_t                              *pool;
    ...
    sin->sin_addr.s_addr = ngx_inet_addr((u_char *)NGX_FAKE_IP, ngx_strlen(NGX_FAKE_IP));

    addrs[0].sockaddr = (struct sockaddr *) sin;
    addrs[0].socklen = sizeof(struct sockaddr_in);

    addrs[0].name.len = ngx_strlen(fake_peer);
    addrs[0].name.data = (u_char *)fake_peer;
    ...
    if (ngx_http_upstream_del_fake_peer_check(cycle, uscf) == NGX_OK) {
        ngx_http_upstream_del_peer(cycle, uscf, &us);
    }
...
static ngx_int_t
ngx_http_upstream_del_fake_peer_check(ngx_cycle_t *cycle,
        ngx_http_upstream_srv_conf_t *uscf)
{
    ngx_uint_t                       i, j;
    ngx_http_upstream_rr_peers_t    *peers = NULL;
    ...
    for (i = 0; i < 1; i++) {
        for (j = 0; j < peers->number; j++) {
            ...
            if (ngx_memcmp(peers->peer[j].name.data, 
                        (u_char *)fake_peer, peers->peer[j].name.len) == 0) {
...
ngx_int_t
ngx_http_upstream_del_server(ngx_cycle_t *cycle, ngx_channel_t *ch)
{
    u_char                                  *port, *p, *last;
    ngx_int_t                                n, j;
    ngx_uint_t                               i;
    ...
    if (ngx_http_upstream_del_peer(cycle, uscf, &us) != NGX_OK) {
        goto invalid;
    }
...
static ngx_int_t
ngx_http_upstream_del_peer(ngx_cycle_t *cycle,
    ngx_http_upstream_srv_conf_t *uscf, ngx_http_upstream_server_t *us)
{
    ...
    if (tmp_peers) {
        ...
        if (upstream_host->update_label) {
            ngx_http_upstream_add_server_event_init(tmp_peers, upstream_host, NGX_DEL);
        }
...
static void
ngx_http_upstream_del_check(ngx_cycle_t *cycle, ngx_channel_t *ch)
{
    ngx_uint_t                       i, j;
    ngx_http_upstream_rr_peers_t    *peers = NULL;
    ngx_http_upstream_srv_conf_t    *uscf, **uscfp;
...
static ngx_int_t
ngx_http_upstream_server_parse_json(u_char *buf, ngx_channel_t *ch)
{
    ngx_int_t        i = 0;
...
static void
ngx_http_upstream_add_server_event_init(ngx_http_upstream_rr_peers_t *tmp_peers, 
        ngx_http_upstream_host_t *upstream_host, ngx_flag_t flag)
{
    ngx_time_t                                  *tp;
    ...
    if (flag == NGX_ADD) {
        delay_event->delay_delete_ev.handler = ngx_http_upstream_add_server_add_delay_delete;
        ...
        delay_event->delay_delete_ev.handler =
            ngx_http_upstream_del_server_del_delay_delete;
        delay_event->delay_delete_ev.log = ngx_cycle->log;
        delay_event->delay_delete_ev.data = delay_event;
        delay_event->delay_delete_ev.timer_set = 0;
...
static void
ngx_http_upstream_add_server_add_delay_delete(ngx_event_t *event)
{
    ngx_uint_t                     i;
    ngx_connection_t              *c;
    ngx_delay_event_t             *delay_event;
...
static void
ngx_http_upstream_del_server_del_delay_delete(ngx_event_t *event)
{
    ngx_uint_t                    i;
    ngx_connection_t              *c;
    ngx_delay_event_t             *delay_event;
...
static void *
ngx_http_upstream_server_create_loc_conf(ngx_conf_t *cf)
{
    ngx_http_upstream_server_loc_conf_t  *conf;
    ...
    conf->add_server = NGX_CONF_UNSET_UINT;
...
static char *
ngx_http_upstream_server_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ngx_http_upstream_server_loc_conf_t *prev = parent;
    ngx_http_upstream_server_loc_conf_t *conf = child;

    ngx_conf_merge_uint_value(conf->add_server, prev->add_server, 0);
...
static ngx_int_t
ngx_http_upstream_server_init_process(ngx_cycle_t *cycle)
{
    ngx_uint_t                                i;
    ngx_http_upstream_host_t                 *upstream_host;
    ngx_http_upstream_srv_conf_t            **uscfp;
    ngx_http_upstream_main_conf_t            *umcf;
    ngx_http_upstream_server_ctx_t           *upstream_conf;

    umcf = ngx_http_cycle_get_module_main_conf(cycle, ngx_http_upstream_module);

    upstream_conf = ngx_pcalloc(cycle->pool, sizeof(*upstream_conf));
    if (ngx_array_init(&upstream_conf->upstream_server, cycle->pool, umcf->upstreams.nelts,
    ...
    for (i = 0; i < umcf->upstreams.nelts; i++) {
        ...
        upstream_host = ngx_array_push(&upstream_conf->upstream_server);
...
static void
ngx_http_upstream_dynamic_free(ngx_http_upstream_rr_peers_t *peers)
{
    if (peers != NULL) {
        ngx_free(peers);
        peers = NULL;
...
void
ngx_http_finalize_request_success(ngx_http_request_t *r)
{
    ngx_int_t                              len;
    ngx_buf_t                             *b = NULL;
...
void
ngx_http_finalize_request_failed(ngx_http_request_t *r)
{
    ngx_int_t                              len;
    ngx_buf_t                             *b = NULL;

    ngx_log_debug0(NGX_LOG_DEBUG, r->connection->log, 0, 
                            "upstream add failed finalize_request_failed");

    len = ngx_strlen(add_failed);

    b = ngx_create_temp_buf(r->pool, len);
    ngx_sprintf(b->pos, "%s", add_failed);
    b->last = b->pos + len;
